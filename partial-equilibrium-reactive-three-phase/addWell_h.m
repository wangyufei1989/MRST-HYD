function W = addWell_h(W, G, rock, cellInx, varargin)
...
W  = [W; struct('cells'   , cellInx(:),           ...
                ...
                'rho',opt.rho,...
            'pressure0',opt.pressure0,...
             'rho0',opt.rho0,...
             'dp',opt.dp,...
                          'mu0',opt.mu0,...
                         'frac',opt.frac,...
                         'mu',opt.mu,'Tk',opt.Tk,'salinity',opt.salinity,'m_NaCl',opt.m_NaCl,'species',opt.species)];

if numel(W(end).dir) == 1,
   W(end).dir = repmat(W(end).dir, [numel(W(end).cells), 1]);
end
% if numel(W(end).pressure) == 1,
%    W(end).pressure = repmat(W(end).pressure, [numel(W(end).cells), 1]);
% end
