...
disco2=0;
% initial pressure changes from 50 bar at the left to 30 bar at the right
...
pB_in=[30*barsa+2e6,30*barsa+pc+2e6];
frac_in=[1 0 0 0];
T_in=278.15;
kr=[0 1];
...
mc=sum((State.rho(:,1).*State.s(:,1).*State.frac(:,2)).*State.poro.*Grid.cells.volumes)+sum((State.rho(:,2).*State.s(:,2)).*State.poro.*Grid.cells.volumes...
    +sum(State.rho(:,3).*(0.3-State.poro).*Grid.cells.volumes).*44/(44+18*6));
mb=sum((State.rho(:,1).*State.s(:,1).*State.frac(:,1)).*State.poro.*Grid.cells.volumes)+sum(State.rho(:,3).*(0.3-State.poro).*Grid.cells.volumes).*18*6/(44+18*6);
co2=0;
water=0;
...
while t<T
    ...
    if  abs(t-dst)<3*dt && State.solve==1&&t<T
        ...
        savei=num2str(jc);
        savex=strcat('s0theo',savei,'.mat');
        save(savex,'State')
        savet=strcat('t0theo',savei,'.mat');
        save(savet,'t')
        jc=jc+1;
    ...
    %% monitor the error
    % net out flux of CO2 component
    co2=co2-State.outflux(2);
    % net out flux of water component
    water=water-State.outflux(1);
    % mass balance of the CO2 component
    err2=(sum((State.rho(:,1).*State.s(:,1).*State.frac(:,2)).*State.poro.*Grid.cells.volumes)+sum((State.rho(:,2).*State.s(:,2)).*State.poro.*Grid.cells.volumes)...
        +sum(State.rho(:,3).*(0.3-State.poro).*Grid.cells.volumes).*44/(44+18*6)-co2 -mc)...
    ...
    err1=(sum((State.rho(:,1).*State.s(:,1).*State.frac(:,1)).*State.poro.*Grid.cells.volumes)+sum(State.rho(:,3).*(0.3-State.poro).*Grid.cells.volumes).*18*6/(44+18*6)...
        -water -mb)...
        /(sum((State.rho(:,1).*State.s(:,1).*State.frac(:,1)).*State.poro.*Grid.cells.volumes)+sum(State.rho(:,3).*(0.3-State.poro).*Grid.cells.volumes).*18*6/(44+18*6));
