function bc = addBC_h(bc, f,varargin)
...
opt = struct('sat', [],'type',[],'value',[],'f_hv',[],'frac',[],'kr',[],'Tk',[]);
opt = merge_options(opt, varargin{:});
s   = opt.sat;

if isempty(bc),
   bc = struct('face', [], 'type', {{}}, 'value', [], 'sat', [],'f_hv',[],'frac',[],'kr',[],'Tk',[]);
end
...
%
if size(opt.sat,1) == 1, opt.sat = opt.sat(ones([nf, 1]), :); end
if numel(opt.value)  == 1, opt.value = opt.value(ones([nf, 1])   ); end

% Verify that v and s are same length as faces (or s empty).
if size(opt.value,2)==1, assert (numel(opt.value)      ==     nf  );end
if size(opt.value,2)==2, assert (numel(opt.value)      ==     nf*2  );end
assert (any(size(opt.sat,1) == [0, nf]));

% Boundary conditions structurally verified.  Append to existing structure.
bc.face  = [bc.face ; f(:)];
bc.type  = [bc.type , opt.type];
bc.value = [bc.value; opt.value];
bc.sat   = [bc.sat  ; opt.sat];
bc.f_hv   = [bc.f_hv  ; opt.f_hv];
bc.frac=[bc.frac;opt.frac];
bc.kr=[bc.kr;opt.kr];
bc.Tk=[bc.Tk;opt.Tk];
