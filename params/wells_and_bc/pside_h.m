function bc = pside_h(bc, G, side, pressure,frac,Tk,KR,state,fluid,varargin)
...
[~,topgrid]=min(G.faces.centroids(:,3));
 pressure=(G.faces.centroids(ix,:)-G.faces.centroids(topgrid,:))*gravity'.*(omega)+pressure;
...
if side(1)=='x' | side(1)=='y'
 ...
 frac=frac(ones([numel(ix), 1]),:);
 kr=KR(ones([numel(ix), 1]),:);
 %Tk=Tk(ones([numel(ix), 1]));
Tk=(G.faces.centroids(ix,3)-G.faces.centroids(topgrid,3))*0.03+Tk;

type='pressure';


bc = addBC_h(bc, ix, 'type',type,'value', pressure, 'f_hv',f_hv,'sat', sat,'frac',frac,'kr',kr,'Tk',Tk);
